...
export async function getServerSideProps({params}: GetServerSidePropsContext) {
    let result = await query("SELECT * FROM shoppinglists WHERE id = $1", [params?.id]);
    const shoppingList: ShoppingList = result.rows[0];

    result = await query("SELECT shoppinglists_recipes.recipe_id AS id, recipes.name, shoppinglists_recipes.serves FROM shoppinglists_recipes JOIN recipes ON shoppinglists_recipes.recipe_id = recipes.id WHERE shoppinglist_id = $1", [params?.id]);
    shoppingList.recipes = result.rows;

    result = await query("SELECT shoppinglists_ingredients.ingredient_id AS id, ingredients.name, units.abbreviation AS unit, shoppinglists_ingredients.amount FROM shoppinglists_ingredients JOIN ingredients ON shoppinglists_ingredients.ingredient_id = ingredients.id JOIN units ON ingredients.unit_id = units.id WHERE shoppinglist_id = $1", [params?.id]);
